...
int main(int argc, char** argv){
	...
	//create a virtual grid and get all the data in each processor 
	int root_np = data_per_np;
	int dims[2] = {0, 0};

	//idk what is this meaning 
	...
	int new_rank;
	MPI_Comm_rank(Cartesian, &new_rank);
	printf("Cartesian new rank : %d\n",new_rank);

	if (new_rank == 0){
		printf("Create Cartesian\n");
	}
	...
	//TODO Change from Scatter with all nums in rows into the virtual grid
	MPI_Scatterv(matrix_A,count,displs,MPI_INT,local_A,recv_count, MPI_INT, 0, Cartesian);

	if(new_rank == 0){
		printf("Sending the Matrix A and Matrix B into each processor\n");
	}
